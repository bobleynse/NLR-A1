...
def constructOutputImage(predictions):
    ...
    for x in range(outputImageXSize):
        for y in range(outputImageYSize):
            if predictions[rowIndex] == 0:
                outputImagePixels[x, y] = (0, 0, 0)
            elif predictions[rowIndex] == 1:
                outputImagePixels[x, y] = (255, 0, 0)
            elif predictions[rowIndex] == 2:
                outputImagePixels[x, y] = (0, 255, 0)
            elif predictions[rowIndex] == 3:
                outputImagePixels[x, y] = (0, 0, 255)
            elif predictions[rowIndex] == 4:
                outputImagePixels[x, y] = (255, 165, 0)
            elif predictions[rowIndex] == 5:
                outputImagePixels[x, y] = (255, 255, 0)
            rowIndex += 1
