...
def writeDataFile(inputImagePath, outputImagePath, inputImageFiles, outputImageFiles, dataFileName):
    ...
    for i in range(len(inputImageFiles)):
        ...
        outputImageHighwayPixelsArr = []
        outputImageMediumroadPixelsArr = []
        outputImageSmallroadPixelsArr = []
        ...
        for x in range(rectSize//2, inputImageXSize - (rectSize//2)):
            for y in range(rectSize//2, inputImageYSize - (rectSize//2)):
                ...
                #print(isRoadPixel)
                if(isRoadPixel >= 248):
                    outputImageHighwayPixelsArr.append((x, y))
                elif(isRoadPixel >= 245 and isRoadPixel < 248):
                    outputImageMediumroadPixelsArr.append((x, y))
                elif(isRoadPixel > 238 and isRoadPixel < 245):
                    outputImageSmallroadPixelsArr.append((x, y))
                else:
        ...
        outputImages = [random.sample(outputImageNonRoadPixelsArr, len(outputImageNonRoadPixelsArr)),
        random.sample(outputImageHighwayPixelsArr, len(outputImageHighwayPixelsArr)),
        random.sample(outputImageMediumroadPixelsArr, len(outputImageMediumroadPixelsArr)),
        random.sample(outputImageSmallroadPixelsArr, len(outputImageSmallroadPixelsArr))]
        ...
        for m in range(linesLimit):
            ...
            outputSelector = (outputSelector + 1) % 4
